...
const TimeAdjustmentHistory: React.FC<TimeAdjustmentProps> = ({ onViewRequest }) => {
    const { activity } = useAppContext();
    const adjustmentRequests = activity.filter(item => item.type === RequestType.TimeAdjustment);
    ...
    return (
        <div className="p-6 bg-light-card dark:bg-dark-card border border-light-border dark:border-dark-border rounded-xl shadow-lg dark:shadow-2xl dark:shadow-black/30 h-full">
            ...
            <div className="space-y-2">
                ...
                adjustmentRequests.map((item: ActivityRequest) => (
                    ...
                    <div>
                        <p className="font-semibold text-sm text-gray-800 dark:text-dark-text">Adjustment for {item.adjustmentDate}</p>
                ...
                <p className="text-gray-600 dark:text-gray-400 text-sm mt-4 text-center">You have no time adjustment history.</p>
                )}
            </div>
        </div>
    );
...
const TimeAdjustment: React.FC<TimeAdjustmentProps> = ({ onViewRequest }) => {
            ...
            <div className="p-4 sm:p-6 bg-light-card dark:bg-dark-card border border-light-border dark:border-dark-border rounded-xl shadow-lg dark:shadow-2xl dark:shadow-black/30 mb-6">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start">
                     <div className="flex items-start space-x-4">
                        <div className="p-2 bg-primary/10 rounded-lg text-primary">{ICONS.timeAdjustment}</div>
                        <div>
                            <h2 className="text-2xl font-bold text-gray-800 dark:text-dark-text">Time Adjustment</h2>
                            <p className="text-sm text-gray-500 dark:text-gray-400">Correct errors in your daily time logs.</p>
                        </div>
                    </div>
                    <button 
                    ...
                    >
                        {ICONS.timeAdjustment}<span>File New Request</span>
                    </button>
                </div>
            </div>

            <TimeAdjustmentHistory onViewRequest={onViewRequest} />

            <TimeAdjustmentModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
        </>
    );
};
