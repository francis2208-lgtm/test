...
import FileChangeScheduleModal from '../modals/FileChangeScheduleModal';

interface ChangeScheduleProps {
  onViewRequest: (request: ActivityRequest) => void;
}

const ChangeScheduleHistory: React.FC<ChangeScheduleProps> = ({ onViewRequest }) => {
    const { activity } = useAppContext();
    const scheduleRequests = activity.filter(item => item.type === RequestType.ScheduleChange);
    ...
    return (
        <div className="p-6 bg-light-card dark:bg-dark-card border border-light-border dark:border-dark-border rounded-xl shadow-lg dark:shadow-2xl dark:shadow-black/30 h-full">
            ...
            <div className="space-y-2">
                ...
                scheduleRequests.map((item: ActivityRequest) => (
                    ...
                    <div>
                        <p className="font-semibold text-sm text-gray-800 dark:text-dark-text">{item.type}</p>
                ...
                <p className="text-gray-600 dark:text-gray-400 text-sm mt-4 text-center">You have no schedule change history.</p>
                )}
            </div>
        </div>
    );
...
const ChangeSchedule: React.FC<ChangeScheduleProps> = ({ onViewRequest }) => {
        ...
        <div className="p-4 sm:p-6 bg-light-card dark:bg-dark-card border border-light-border dark:border-dark-border rounded-xl shadow-lg dark:shadow-2xl dark:shadow-black/30 mb-6">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start">
                 <div className="flex items-start space-x-4">
                    <div className="p-2 bg-primary/10 rounded-lg text-primary">{ICONS.schedule}</div>
                    <div>
                        <h2 className="text-2xl font-bold text-gray-800 dark:text-dark-text">Schedule Change</h2>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Request adjustments to your work schedule.</p>
                    </div>
                </div>
                <button 
                ...
                >
                    {ICONS.schedule}<span>File New Request</span>
                </button>
            </div>
        </div>

        <ChangeScheduleHistory onViewRequest={onViewRequest} />
        
        <FileChangeScheduleModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
    </>
  );
};
